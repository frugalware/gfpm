...
void gfpm_create_pkgs_treeview(void)
{
	...
	renderer = gtk_cell_renderer_text_new();
	gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(pkgs_treeview),
				-1, "Package Name", renderer, "text", 2,
				NULL);

	renderer = gtk_cell_renderer_text_new();
	gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(pkgs_treeview),
				-1, "Version", renderer, "text", 3,
				NULL);

	renderer = gtk_cell_renderer_text_new();
	gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(pkgs_treeview),
				-1, "Remote Version", renderer, "text", 4,
				NULL);

	renderer = gtk_cell_renderer_text_new();
	gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(pkgs_treeview),
				-1, "Description", renderer, "text", 5,
				NULL);
...
void _load_info_treeview(char *pkgname)
{
	...
	gtk_list_store_append(GTK_LIST_STORE(model), &iter);
	gtk_list_store_set(GTK_LIST_STORE(model), &iter,
			0, "Name:", 1, (char *)alpm_pkg_getinfo(pkg, PM_PKG_NAME),
			-1);

	gtk_list_store_append(GTK_LIST_STORE(model), &iter);
	gtk_list_store_set(GTK_LIST_STORE(model), &iter,
			0, "Version:", 1, (char *)alpm_pkg_getinfo(pkg, PM_PKG_VERSION),
			-1);
	...
	if (r != 0)
	{
		gtk_list_store_append(GTK_LIST_STORE(model), &iter);
		gtk_list_store_set(GTK_LIST_STORE(model), &iter,
				0, "Packager:", 1, (char *)alpm_pkg_getinfo(pkg, PM_PKG_PACKAGER),
				-1);
	}
	...
	if (r != 0)
	{
		gtk_list_store_append(GTK_LIST_STORE(model), &iter);
		gtk_list_store_set(GTK_LIST_STORE(model), &iter,
				0, "URL:", 1, (char *)alpm_pkg_getinfo(pkg, PM_PKG_URL),
				-1);
	}
	...
	if (r != 1)
	{
		...
		gtk_list_store_set(GTK_LIST_STORE(model), &iter,
				0, "Size (Compressed):", 1, (char *)tmp,
				-1);
		gtk_list_store_append(GTK_LIST_STORE(model), &iter);
		asprintf(&tmp, "%ld", (long)alpm_pkg_getinfo(pkg, PM_PKG_USIZE));
		gtk_list_store_set(GTK_LIST_STORE(model), &iter,
				0, "Size (Uncompressed):", 1, (char *)tmp,
				-1);
	}
	...
	if (r != 1)
	{
		gtk_list_store_append(GTK_LIST_STORE(model), &iter);
		gtk_list_store_set(GTK_LIST_STORE(model), &iter,
				0, "SHA1SUM:", 1, (char *)alpm_pkg_getinfo(pkg, PM_PKG_SHA1SUM),
				-1);
	}
	...
	if (r != 0)
	{
		...
		gtk_list_store_append(GTK_LIST_STORE(model), &iter);
		gtk_list_store_set(GTK_LIST_STORE(model), &iter,
				0, "Required by:", 1, (char *)foo->str,
				-1);
	}

	gtk_list_store_append(GTK_LIST_STORE(model), &iter);
	gtk_list_store_set(GTK_LIST_STORE(model), &iter,
			0, "Description:", 1, (char *)alpm_pkg_getinfo(pkg, PM_PKG_DESC),
			-1);
}
